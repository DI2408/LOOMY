...
export type CourierDispatchSystemDeps = {
  ...
  /** Max km from dropoff when coords exist on order + courier. Default 15. */
  maxRadiusKm?: number;
};

/**
 * Broadcasts new pickup jobs to eligible couriers; atomic claim via repository.
 */
export class CourierDispatchSystem {
  ...
  /**
   * Optional: listen for in-process `order.ready_for_pickup` (e.g. workers).
   * Route handlers typically call `broadcastOrderToCouriers` via OrderManager hook directly.
   */
  listen(): void {
  ...
  /**
   * Called when order becomes `ready_for_pickup`: notify filtered active couriers (SSE / future push).
   */
  ...
  /**
   * First successful DB claim wins: `ready_for_pickup` + `courier_id` IS NULL → `dispatched` + courier.
   */
