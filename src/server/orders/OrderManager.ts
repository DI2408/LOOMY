...
export type OrderManagerAgentHooks = {
  /** After transition to `paid`: e.g. email/SMS kvittering til kunden. */
  sendCustomerConfirmation: (orderId: string) => Promise<void>;
  /** After transition to `paid`: aktiverer butiks-agenten (Realtime, kø, intern bus). */
  notifyStoreAgent: (orderId: string) => Promise<void>;
  /** After transition to `ready_for_pickup`: bud-system / broadcast til Bude-agent. */
  notifyCourierSystem: (orderId: string) => Promise<void>;
  /** After transition to `delivered`: afslutning, analytics, osv. */
...
/**
 * LOOMY OrderManager — state machine with DB persistence and agent triggers.
 *
 * Brug service role Supabase-klient fra Edge Function eller server route, eller
 * indsæt RLS-politikker der matcher jeres roller.
 */
export class OrderManager {
  ...
  /**
   * Opret række til `pending_payment` (fx efter Stripe intent oprettet, før capture).
   */
  async registerPendingOrder(orderId: string): Promise<OrderManagerRow> {
    const existing = await this.repo.getById(orderId);
    if (existing) {
      return existing;
    }
    return this.repo.insertPending(orderId);
  }

  /**
   * pending_payment → paid: gemmer status, sender kundebekræftelse, notifier butik.
   */
  ...
  /**
   * paid → ready_for_pickup: gemmer status, besked til bud-system.
   */
  ...
  /**
   * ready_for_pickup → delivered: afslutter ordren i databasen og hos agenter.
   */
