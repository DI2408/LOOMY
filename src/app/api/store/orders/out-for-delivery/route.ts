...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!orderId || !storeId) {
    return new Response(
      JSON.stringify({ error: "orderId and storeId required" }),
  ...
  try {
    ...
    const { createSupabaseOrderManagerRepository } = await import(
      "@/server/orders/orderManagerRepository"
    );
    ...
    const updated = await getOrderManager().transitionToOutForDelivery(orderId);
    ...
    if (e instanceof OrderTransitionError) {
      return new Response(
        JSON.stringify({ error: e.message, currentStatus: e.currentStatus }),
