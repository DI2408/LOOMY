...
export function LumiShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-x-hidden bg-[#faf8f5]">
      <div
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-40 border-t-[0.5px] border-stone-200/90 bg-[#faf8f5]/85 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] backdrop-blur-md md:hidden"
              ...
              >
                <motion.span
                  ...
                  className={`flex h-10 w-10 items-center justify-center rounded-xl border-[0.5px] transition-colors ${
                    active
                      ? "border-[#8b6914]/35 bg-[#8b6914]/10 text-[#6b4f0a]"
                      : "border-transparent bg-white/60 text-stone-600"
                  }`}
                >
